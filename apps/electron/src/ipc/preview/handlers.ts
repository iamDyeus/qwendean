...
export function registerPreviewHandlers() {
  ipcMain.removeHandler('preview:create-window');
  ipcMain.removeHandler('preview:close-window');
  ...
  ipcMain.handle('preview:create-window', async (_event, projectId: string) => {
    ...
    try {
      ...
      previewWindow.loadURL(`data:text/html,
        <html><body style="display:flex;align-items:center;justify-content:center;height:100vh;font-family:sans-serif;background:#1a1a1a;color:#fff;">
  ...
  ipcMain.handle('preview:reload', async () => {
    if (previewWindow && !previewWindow.isDestroyed()) previewWindow.reload();
    return { success: true };
  });
}
